        ...
        <div class="my-container">
            ...
            <div class="profile_sidebar_grid_area">
                ...
                <div class="profile_content_wrapper">
                    ...
                    <div class="order_product_item_area review_order_item_area" style="text-align: center; margin-top: 25px;">
                        <button class="support_btn" id="modalClickButton1">
                            <i class="fa fa-plus" style="font-size: 25px; margin-bottom: 10px;"></i>
                            <br>
                            {{ __('customer.create_a_ticket') }}
                        </button>
                    </div>
                    <br><br>
                            ...
                            <table>
                                <tr>
                                    <td style="text-align: left;">
                                        <h3>{{ __('customer.id') }}</h3>
                                    </td>
                                    <td>
                                        <h3>{{ __('customer.subject') }}</h3>
                                    </td>
                                    <td>
                                        <h3>{{ __('customer.message') }}</h3>
                                    </td>
                                    <td>
                                        <h3>{{ __('customer.status') }}</h3>
                                    </td>
                                    <td>
                                        <h3>{{ __('customer.action') }}</h3>
                                    </td>
                                </tr>
                                @if ($support_tickets->count() > 0)
                                    @foreach ($support_tickets as $ticket)
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{ route('customer.supportDetails', ['id'=>$ticket->id]) }}" class="view_list_btn"><i class="fa-solid fa-eye"></i></a>
                                            </td>
                                        </tr>
                                    @endforeach
                                @else
                ...
                <div wire:ignore.self class="modal_wrapper" id="modalID1">
                    <div class="modal_dialog">
                        <div class="modal_header d-flex align-items-center justify-content-between flex-wrap-wrap g-sm">
                            <h3 style="font-size: 17px;"> {{ __('customer.create_new_ticket') }}</h3>
                            <button type="button" id="modalClose1">
                                <i class="fa fa-times" style="font-size: 25px;"></i>
                            </button>
                        </div>
                        <div class="modal_body">
                            <form wire:submit.prevent='storeTicket' class="form_area">
                                <div class="input_row">
                                    <label for="" style="font-size: 13px;"> {{ __('customer.subject') }}</label>
